...
When the program is initialised, the program creates a 'downloads' folder with relevant subdirectories. This is handled within the ``__main__`` function and is why the script should be run with administrative privileges. If you are uncomfortable doing this, please create the following directories in the current folder downloads, downloads/binary and downloads/text and comment out the ``os.makedirs`` lines in the ``__main__`` function. 
...
- Logging the timestamp and URL the script is connecting to. 
...
``subdirectory_count`` - The subdirectory count **DOES NOT** include errored directories. These may be either malformed, OR have returned an error type of three. This **DOES** include empty directories, as they are still valid. 
``text_file_count`` & ``text_files_list`` - The text file count and list **DOES NOT** included errored files. While the script still attempts to download these files, they are excluded from the results.
``binary_count`` & ``binary_files_list`` - The binary count and list **DOES NOT** include errored files.
